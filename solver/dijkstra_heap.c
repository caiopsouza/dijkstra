...
void
set_explored_heap(bool is_explored[], size_t costs[], size_t explored[], size_t *explored_next, size_t vertex,
                  size_t cost) {
...
void update_neighbors_heap(Graph graph, size_t vertex, bool is_explored[], size_t costs[]) {
...
void dijkstra_heap(Graph graph, size_t costs[]) {
    ...
    size_t start = 0;
    set_explored_heap(is_explored, costs, explored, &explored_next, start, 0);
    update_neighbors_heap(graph, start, is_explored, costs);
    ...
    while (explored_next < graph->vertices) {
        ...
        set_explored_heap(is_explored, costs, explored, &explored_next, smallest_vertex, smallest_cost);
        update_neighbors_heap(graph, smallest_vertex, is_explored, costs);
    }
}
